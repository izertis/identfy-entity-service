  ...
  if (
    ...
    if (!credentialStatus.statusListCredential) {
      return {
        valid: false,
        error:
          'StatusList2021Entry must contain a statusListCredential parameter',
      };
    }
    let fetchResponse;
    try {
      fetchResponse = await fetch(credentialStatus.statusListCredential);
      if (fetchResponse.status !== 200) {
        return {
          valid: false,
          error: `CredentialStatusList could not be recovered - Error code ${
    ...
    if (!jwt.payload.vc.validFrom) {
      return {
        valid: false,
        error: 'StatusVC does not have a validFrom parameter',
      };
    }
    ...
    if (validFrom > now + clockTolerance) {
      return {
        valid: false,
        error: `StatusVc not yet valid: ${validFrom} > ${now}`,
      };
    }
    const expDate = Math.floor(
      Date.parse(jwt.payload.vc.expirationDate) / 1000,
    );
    if (expDate <= now) {
    ...
    if (!(jwt.payload.vc.type as string[]).includes(STATUS_LIST_2021_VC)) {
      return {
        valid: false,
        error: `StatusVc is not of type ${STATUS_LIST_2021_VC}`,
      };
    }
    if (
      jwt.payload.vc.credentialSubject.statusPurpose !==
    ...
    const indexValue = getBitFromBitString(
      gzipUncodedList,
      parseInt(credentialStatus.statusListIndex),
    );
    if (indexValue === 1) {
