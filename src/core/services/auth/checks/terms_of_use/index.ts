...
import {
  ChainCheckAccreditationType
} from './accreditation_chain.js';
import {
  AsyncChainHandler, Context, HandlerWithContext
} from '../../../../../shared/classes/utility/handler.js';
import { VerificationResult, W3CTermsOfUse, W3CVerifiableCredential } from 'openid-lib';
...
export async function checkTrustChain(jwtVc: W3CVerifiableCredential): Promise<VerificationResult> {
  const resolve = async (term: W3CTermsOfUse): Promise<VerificationResult> => {
    ...
    const checkResult = new ChainCheckAccreditationType(jwtVc.type).isAccreditation().collect();
    const context = Context.fromObject({ deferenceData, originalVc: jwtVc });
    let taoHandler, tiHandler = undefined;
  ...
  return new class extends HandlerWithContext {
    handle(context: Context): Promise<void> {
      return new Promise(async (resolve, _reject) => {
        const deferenceDataPointer = context.getMutOrThrow<DerefenceData>("deferenceData");
...
function generateTaoHandlerForAccreditation() {
  return new class extends HandlerWithContext {
    handle(context: Context): Promise<void> {
      return new Promise(async (resolve, _reject) => {
        const deferenceDataPointer = context.getMutOrThrow<DerefenceData>("deferenceData");
        const certificateVc = await getFromUrl(deferenceDataPointer.value.jwtVc.vc.termsOfUse.id);
        deferenceDataPointer.value = tirDereference(certificateVc);
        resolve();
      });
    }
...
function tirDereference(tirEntry: TirAttributeResponse): DerefenceData {
  ...
  if (tirEntry.did !== jwt.vc.credentialSubject.id) {
    throw new Error("VC was issued for a different entity that the one specified in the registry")
  }
  const check = new ChainCheckAccreditationType(jwt.vc.type).isAccreditation().unique();
...
function analyzeAttestationVc(
  ...
  for (const accreditedFor of accreditVc.vc.accreditedFor) {
    if (accreditedFor.schemaId === originalVc.vc.credentialSchema.id) {
      if ((originalVc.vc.type as Array<string>).every((x) => accreditedFor.types.includes(x))) {
        resultFound = true;
        break;
      }
    }
  ...
  if (!resultFound) {
    throw new Error("Invalid issuance certificate detected in trust chain. Issuer not allowed to issue VC");
  }
    ...
    switch (deferenceData.issuerType) {
      case IssuerType.RTAO:
        if (
          ...
          return {
            valid: false,
            error: "RTAO accreditations can only be issued by the EBSI Support Office"
          };
        }
        return { valid: true };
      case IssuerType.TAO:
        await taoHandler.handle(context);
        return resolveAccreditation(
          context,
          taoHandler,
          tiHandler
        );
