...
export default class AuthSchema {
  getConfiguration = Joi.object({
    issuerUri: Joi.string().required()
  });
  authorize = Joi.object({
    response_type: Joi.string().when(
      "request",
      {
        is: Joi.exist(),
        then: Joi.optional(),
    ...
    request: Joi.string().description("Authz request in JWT format used by Service Wallets")
  });
  ...
  token = Joi.object({
    ...
    user_pin: Joi.string().optional().description("Pin of the User"),
    "pre-authorized_code": Joi.string().optional().description("Code for pre-authorised flow"),
    client_assertion: Joi.string().when(
    ...
    client_assertion_type: Joi.string().optional().description("The assertion type")
      .valid("urn:ietf:params:oauth:client-assertion-type:jwt-bearer"),
    // Custom data
    issuerUri: Joi.string().required(),
    privateKeyJwk: Joi.string().required(),
    publicKeyJwk: Joi.string().required(),
  ...
  presentation = Joi.object({
    ...
    privateKeyJwk: Joi.string().required(),
    publicKeyJwk: Joi.string().required(),
    verify_flow: Joi.object({
      scope: Joi.string().required().description("Scope").default("openid"),
      response_type: Joi.string().required(),
      presentation_definition: Joi.object({
        id: Joi.string().required(),
        input_descriptors: Joi.array().required(),
        name: Joi.string().optional(),
        purpose: Joi.string().optional(),
        format: Joi.object().required()
      })
    }),
    state: Joi.string().optional().description("Identifier of session"),
  });
}
